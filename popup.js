...
function formatCode(code) {
    //reads input into input.js
    fs.writeFile(
        __dirname,"/tests/input.js",
        code,
        function(err){
            if(err){
    ...
    //spawns powershell script to deobfuscate it
    spawn("powershell.exe",['.\deobfuscate.ps1]'])
    //saves result to ouput.js and reads it into popup.js
    var code=fs.readFile('/tests/output.js',function (err,data){
        if(err){
            return console.error(err)
        }
        var ouput=data.ToString
        console.log('data read:'+output)
        return output
    })
